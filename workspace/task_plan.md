...
1. **草稿的显式删除入口**（P1，体验+数据卫生）。现在只有「关向导时空草稿丢弃」这一条删除路径；dev 上已积了 3 条草稿（`77`、`Codex巡检-可删`、`122`），其中 `Codex巡检-可删` 是 09-13 巡检留下的 0 科测试草稿。做法：草稿行/详情抽屉加「删除草稿」，复用 `removeMajorById(major, '丢弃草稿「x」')`，并记审计。
2. **dev 遗留数据清理**：上面三条草稿（依赖第 1 项）＋ `exam_records` 里已删考试留下的孤儿行（`0c6b0c5` 已让草稿板块不再展示，行仍在表里）。可写一个一次性清理脚本或接口。
...
- [ ] 方案 4 筛选与分页（创建人收进更多筛选、分页 12/25/50、周测前 5 条 + 还有 N 条）
- [ ] 方案 3 视图/密度切换（按考试 / 按班级）
