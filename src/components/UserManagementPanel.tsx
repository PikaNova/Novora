...
export default function UserManagementPanel({ grades, classes, currentUser, forcePasswordChange = false }: Props) {
  ...
  const submitBatchUsers = async () => {
    ...
    try {
      ...
      setUsers(latest); setBatchUserDraft(null); setMessage(`已创建 ${completed} 个班级管理员账号，首次登录均需修改密码。`);
  ...
  const submitReset = async () => {
    ...
    setBusy(true);
    try { await resetManagedUserPassword(resetTarget.id, resetPassword); setResetTarget(null); setResetPassword(''); setResetError(''); setMessage('密码已重置，该用户需要重新登录并修改密码。'); }
