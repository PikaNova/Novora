...
export default function UserManagementPanel({ grades, classes, currentUser, forcePasswordChange = false, openBatchCreate = false }: Props) {
  ...
  const removeUser = async (user: ManagedUser) => {
    if (!await confirmDialog({ title: `删除管理员“${user.displayName}”`, message: '该账号会立即退出所有设备，且无法恢复。', tone: 'danger', confirmLabel: '删除管理员' })) return;
    try { setUsers(await deleteManagedUser(user.id)); setMessage(`管理员“${user.displayName}”已删除。`); }
    catch (error) { setMessage(error instanceof Error ? error.message : '删除用户失败'); }
  };
  const removeRole = async (role: ManagedRole) => {
    if (!await confirmDialog({ title: `删除角色“${role.name}”`, message: '删除后无法再将该角色分配给管理员。', tone: 'danger', confirmLabel: '删除角色' })) return;
    try { setRoles(await deleteManagedRole(role.id)); }
