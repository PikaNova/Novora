...
export default function UserManagementPanel({ grades, classes, currentUser, forcePasswordChange = false }: Props) {
  ...
  return <main className="user-management">
    ...
    {userDraft && <div className="admin-modal-overlay"><div className="admin-modal admin-modal--wide" onClick={event => event.stopPropagation()}><h2 className="admin-modal__title">{userDraft.id ? '编辑管理员' : '添加管理员'}</h2><div className="user-editor__grid"><label className="admin-label">用户名<input className="admin-input" disabled={!!userDraft.id} value={userDraft.username} onChange={event => setUserDraft(value => value && { ...value, username: event.target.value })} placeholder="如：grade3_admin" /></label><label className="admin-label">显示名称<input className="admin-input" value={userDraft.displayName} onChange={event => setUserDraft(value => value && { ...value, displayName: event.target.value })} placeholder="如：高三教务" /></label>{!userDraft.id && <label className="admin-label">初始密码<input className="admin-input" type="password" value={userDraft.password} onChange={event => setUserDraft(value => value && { ...value, password: event.target.value })} placeholder="至少 8 位" /></label>}<label className="admin-label">角色<select className="admin-input" value={userDraft.roleId} onChange={event => setUserDraft(value => value && { ...value, roleId: event.target.value })}>{roles.map(role => <option key={role.id} value={role.id}>{role.name}</option>)}</select></label>{userDraft.id && <label className="admin-label">状态<select className="admin-input" value={userDraft.status} onChange={event => setUserDraft(value => value && { ...value, status: event.target.value as UserDraft['status'] })}><option value="active">启用</option><option value="disabled">停用</option></select></label>}</div><div className="user-editor__scope"><label className="admin-toggle-label"><input type="checkbox" checked={userDraft.allScope || userDraft.roleId === 'super_admin'} disabled={userDraft.roleId === 'super_admin'} onChange={event => setUserDraft(value => value && { ...value, allScope: event.target.checked })} />管理全校数据</label>{!userDraft.allScope && userDraft.roleId !== 'super_admin' && <><h3>可管理年级</h3><div className="admin-major-targets">{grades.map(grade => <label key={grade.id}><input type="checkbox" checked={userDraft.gradeIds.includes(grade.id)} onChange={event => setUserDraft(value => value && { ...value, gradeIds: event.target.checked ? [...value.gradeIds, grade.id] : value.gradeIds.filter(id => id !== grade.id) })} />{grade.name}</label>)}</div><h3>额外指定班级</h3><div className="admin-major-targets">{classes.map(item => <label key={item.id}><input type="checkbox" checked={userDraft.classIds.includes(item.id)} onChange={event => setUserDraft(value => value && { ...value, classIds: event.target.checked ? [...value.classIds, item.id] : value.classIds.filter(id => id !== item.id) })} />{grades.find(grade => grade.id === item.gradeId)?.name} · {item.name}</label>)}</div></>}</div><div className="admin-modal__actions"><button className="admin-btn admin-btn--primary" disabled={busy} onClick={() => void submitUser()}>{busy ? '保存中…' : '保存'}</button><button className="admin-btn" onClick={() => setUserDraft(null)}>取消</button></div></div></div>}
    {roleDraft && <div className="admin-modal-overlay"><div className="admin-modal admin-modal--wide" onClick={event => event.stopPropagation()}><h2 className="admin-modal__title">{roleDraft.id ? '编辑自定义角色' : '新建自定义角色'}</h2><label className="admin-label">角色名称<input className="admin-input" value={roleDraft.name} onChange={event => setRoleDraft(value => value && { ...value, name: event.target.value })} /></label><label className="admin-label">角色职责说明<textarea className="admin-textarea" rows={2} value={roleDraft.description} onChange={event => setRoleDraft(value => value && { ...value, description: event.target.value })} placeholder="说明该角色负责什么、不能做什么，分配账号时会直接展示。" /></label><div className="role-editor__groups">{groupedPermissions.map(group => <section key={group.prefix}><div><strong>{group.label}</strong><button type="button" onClick={() => setRoleDraft(value => value && ({ ...value, permissions: group.items.every(item => value.permissions.includes(item)) ? value.permissions.filter(item => !group.items.includes(item)) : [...new Set([...value.permissions, ...group.items])] }))}>{group.items.every(item => roleDraft.permissions.includes(item)) ? '取消全选' : '全选'}</button></div>{group.items.map(permission => <label key={permission}><input type="checkbox" checked={roleDraft.permissions.includes(permission)} onChange={event => setRoleDraft(value => value && ({ ...value, permissions: event.target.checked ? [...value.permissions, permission] : value.permissions.filter(item => item !== permission) }))} /><span><strong>{permissionMeta(permission).label}</strong><small>{permissionMeta(permission).description}</small></span></label>)}</section>)}</div><div className="admin-modal__actions"><button className="admin-btn admin-btn--primary" disabled={busy} onClick={() => void submitRole()}>{busy ? '保存中…' : '保存角色'}</button><button className="admin-btn" onClick={() => setRoleDraft(null)}>取消</button></div></div></div>}
