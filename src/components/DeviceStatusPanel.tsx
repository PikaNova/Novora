...
export default function DeviceStatusPanel({ canRevoke = true }: { canRevoke?: boolean }) {
  ...
  return <main className="device-status">
    ...
    <div className="device-status__toolbar"><label><span>搜索</span><input className="admin-input" value={query} onChange={event => setQuery(event.target.value)} placeholder="看板实例、插件实例、班级或考试" /></label><label><span>年级</span><InlineSelect className="admin-input" value={gradeFilter} onChange={value => { setGradeFilter(value); setClassFilters([]); }} options={[{ value: '*', label: '全部年级' }, ...grades.map(item => ({ value: item.id, label: item.name }))]} /></label><details className="device-status__class-filter"><summary>班级筛选{classFilters.length ? ` · ${classFilters.length} 个` : ' · 全部'}</summary><ClassMultiPicker options={pickerOptions} selectedIds={classFilters} onChange={setClassFilters} /></details></div>
