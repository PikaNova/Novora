...
export default function InitializationWizard({ open, onClose, onComplete }: Props) {
  ...
  return <div className="init-overlay" role="dialog" aria-modal="true" aria-labelledby="init-title"><div className="init-window">
    ...
    <main className="init-body">
      {step === 0 && <div className="init-form"><label><span>省份 / 地区</span><select autoFocus value={province} onChange={event => setProvince(event.target.value)}><option value="">请选择省份或地区</option>{CHINA_PROVINCES.map(item => <option key={item} value={item}>{item}</option>)}</select></label><label><span>学校名称</span><input value={schoolName} onChange={event => setSchoolName(event.target.value)} placeholder="如：第一中学" maxLength={80} /></label><div className="init-school-fullname"><span>完整校名</span><strong>{schoolFullName(province, schoolName) || '选择省份并填写学校名称后自动生成'}</strong></div><p className="init-note">完整校名将显示在排班预览和 A4 PDF 中，并在你同意遥测后与省份一起上报作者端。</p></div>}
      {step === 1 && <div className="init-school"><p>每行对应一个年级。可直接填写班级，也可输入数量快速创建 1 班至 X 班。</p>{school.map((row, index) => <div className="init-school-row" key={index}><input value={row.name} onChange={event => setSchool(list => list.map((item, i) => i === index ? { ...item, name: event.target.value } : item))} placeholder="年级，如：高一" /><input value={row.classes} onChange={event => setSchool(list => list.map((item, i) => i === index ? { ...item, classes: event.target.value } : item))} placeholder="班级，如：1班、2班" /><label className="init-quick-count"><input type="number" min="1" max="99" value={quickCounts[index] ?? '10'} onChange={event => setQuickCounts(value => ({ ...value, [index]: event.target.value }))} aria-label={`${row.name || '当前年级'}班级数量`} /><button type="button" onClick={() => quickClasses(index, Number(quickCounts[index] || 1))}>生成</button></label><button onClick={() => setSchool(list => list.filter((_, i) => i !== index))} aria-label="删除此年级">×</button></div>)}<button className="init-add" onClick={() => setSchool(list => [...list, { name: '', classes: '' }])}>添加年级</button></div>}
