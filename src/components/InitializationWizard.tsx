...
export default function InitializationWizard({ open, onClose, onComplete, onFinalized }: Props) {
  ...
  return <div className="init-overlay" role="dialog" aria-modal="true" aria-labelledby="init-title"><div className="init-window">
    ...
    <main className="init-body">
      ...
      {step === 2 && <div className="init-form"><label><span>学期开始日期</span><input type="date" value={termStart} onChange={event => setTermStart(event.target.value)} /></label><label><span>周次模式</span><InlineSelect className="init-inline-select" value={weekMode} onChange={value => setWeekMode(value as WeeklyWeekMode)} options={[{ value: 'single', label: '统一周表' }, { value: 'ab', label: 'A/B 周交替' }]} /></label><label className="init-check"><input type="checkbox" checked={excludeOfficialHolidays} onChange={event => setExcludeOfficialHolidays(event.target.checked)} />自动排除法定节假日</label><label><span>默认运行模式</span><InlineSelect className="init-inline-select" value={scheduleMode} onChange={value => setScheduleMode(value as ScheduleMode)} options={[{ value: 'major-only', label: '仅大型考试' }, { value: 'weekly-only', label: '仅周测' }, { value: 'automatic', label: '自动调度' }]} /></label></div>}
