...
export default function TimeRangePickerModal({
  ...
  const validRange =
    duration > 0 &&
    (mode === 'time' ? crossDayEnabled || rawDuration > 0 : crossDayEnabled ? endDate > startDate : !datesDiffer);
  ...
  const display = (value: string) => {
    ...
    return mode === 'time'
      ...
      : `${parts.date} ${pad(parts.hour)}:${pad(parts.minute)}`;
  };
  ...
  const applyPreset = (minutes: number) => {
    ...
    const crossesDay = mode === 'datetime' ? end.date !== start.date : start.hour * 60 + start.minute + minutes >= 1440;
    setDraftEnd(nextEnd);
      ...
      <section ref={modalRef} className="time-range-modal" role="dialog" aria-modal="true" aria-label={title}>
        <header className="time-range-head">
          <div>
            <h2>{title}</h2>
            <p>
              步骤 {stepNumber} / {stepCount}
            </p>
          </div>
          <button type="button" onClick={cancelAndRevert}>
            取消
          </button>
        </header>
            ...
            <div>
              {presets.map((minutes) => (
                <button
                  type="button"
                  key={minutes}
                  className={validRange && duration === minutes ? 'is-selected' : ''}
        ...
        {allowCrossDay && (step === 'end' || (step === 'duration' && requiresCrossDay)) && (
          <section className={`time-range-cross-day${crossDayEnabled ? ' is-enabled' : ''}`}>
            <label>
              <input
                type="checkbox"
                checked={crossDayEnabled}
                onChange={(event) => setCrossDayEnabled(event.target.checked)}
              />
              <span>
                <strong>启用跨日考试</strong>
                <small>仅当考试确实在次日结束时启用</small>
              </span>
            </label>
            {crossDayEnabled && (
              <div className="time-range-cross-day__dates">
                <span>
                  <small>开始日期</small>
                  <strong>{mode === 'datetime' ? startDate : contextLabel || '当天'}</strong>
                </span>
                <span>
                  <small>结束日期</small>
                  <strong>{mode === 'datetime' ? endDate : nextDateLabel(contextLabel)}</strong>
                </span>
              </div>
            )}
          </section>
        ...
        {(step === 'start' || step === 'end') && (
          <section className="time-range-wheel-panel">
            <div>
              <strong>设置{target === 'start' ? '开始' : '结束'}时间</strong>
              <span>上下滚动小时和分钟</span>
            </div>
            <div className="time-range-wheels">
              <div className="time-range-wheel">
                <span>时</span>
                <WheelColumn
                  itemHeight={ITEM_HEIGHT}
                  values={HOURS}
                  value={activeParts.hour}
                  onChange={(hour) => updatePart('hour', hour)}
                  ariaLabel="选择小时"
                />
              </div>
              <b>:</b>
              <div className="time-range-wheel">
                ...
                <WheelColumn
                  ...
                  onChange={(minute) => updatePart('minute', minute)}
                  ariaLabel="选择分钟"
                />
              </div>
            </div>
        ...
        {step === 'duration' && (
          <button
            type="button"
            className="time-range-custom-end"
            onClick={() => {
              setTarget('end');
              setStep('end');
            }}
        ...
        <footer>
          {step === 'start' ? (
            ...
            <button
              ...
              onClick={() => {
                setTarget('start');
                setStep(step === 'end' && !directEndSelection ? 'duration' : 'start');
              }}
            >
              上一步
            </button>
          ...
          {step === 'start' ? (
            <button
              ...
              onClick={() => {
                if (directEndSelection) setTarget('end');
                setStep(directEndSelection ? 'end' : 'duration');
              }}
            >
              {directEndSelection ? '设置结束时间' : '选择时长'}
            </button>
          ) : (
            <button
              type="button"
              ...
              onClick={() => {
                if (validRange) onConfirm(draftStart, draftEnd, crossDayEnabled);
              }}
            >
              确认时间
            </button>
