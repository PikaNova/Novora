...
const SUBJECT_TEMPLATES: SubjectTemplate[] = [
  ...
  },
  {
    id: "humanities",
    name: "文科组合",
    description: "政治、历史、地理",
    subjects: ["政治", "历史", "地理"],
  },
  {
    id: "custom",
    name: "自定义科目组",
    description: "自行增删科目后生成",
    subjects: ["语文", "数学", "英语"],
  },
];
...
function buildDraftItems(subjects: string[], startDate: string, pattern: DayPattern): BatchDraftItem[] {
  return subjects.map((subject, index) => {
    const dayOffset = Math.floor(index / pattern.slots.length);
...
export default function MajorBatchAddModal({
  ...
  const pattern = DAY_PATTERNS.find((item) => item.id === patternId) ?? DAY_PATTERNS[0];
                  ...
                  <div className="major-batch-template-grid">
                    {SUBJECT_TEMPLATES.map((item) => (
                      ...
                      >
                        ...
                        <span>{item.subjects.length} 场</span>
              ...
              {step === 1 && (
                <div className="admin-workflow-pane">
                  ...
                  <section className="major-batch-patterns">
                    <strong>每日场次模板</strong>
                    <div className="quick-major-choice-grid">
                      {DAY_PATTERNS.map((item) => (
                        <button
                          key={item.id}
                          type="button"
                        ...
                        >
                          <strong>{item.name}</strong>
                          <span>{item.slots.length} 场/天</span>
                        </button>
                      ))}
                    </div>
                  </section>
                  <div className="admin-workflow-review">
                    ...
                    <span>
                      预计日期
                      <strong>
                        {fmtDate(startDate)} 起，约 {Math.ceil(subjects.length / pattern.slots.length)} 天
                      </strong>
                    </span>
                    <span>
                      排布规则
                          ...
                          {group.items.map((item) => {
                            ...
                            return (
                              <article key={item.id} className={messages.length ? "has-error" : ""}>
                                ...
                                <span>{durationText(startIso, endIso)}</span>
