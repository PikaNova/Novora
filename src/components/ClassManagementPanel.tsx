...
export default function ClassManagementPanel({ grades, classes, weeklyPlans, majors, onAddGrade, onRemoveGrade, onAddClass, onRemoveClass }: Props) {
  ...
  return <main className="class-management">
    <div className="device-status__heading"><div><h2>年级与班级</h2><p>先建立年级，再在年级下管理班级。客户端将按两级选项完成绑定。</p></div></div>
    <div className="class-management__add"><input className="admin-input" value={gradeName} onChange={event => setGradeName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') addGrade(); }} placeholder="如：高三" /><button className="admin-btn admin-btn--primary" onClick={addGrade}>添加年级</button></div>
    {orderedGrades.length === 0 ? <div className="admin-empty"><p>首次使用请先添加年级。</p></div> : <>
      <div className="device-status__toolbar"><label><span>当前年级</span><select className="admin-input" value={gradeId} onChange={event => setSelectedGradeId(event.target.value)}>{orderedGrades.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button className="admin-btn admin-btn--danger" onClick={() => removeGrade(gradeId, selectedGrade.name)}>删除年级</button></div>
      <div className="class-management__add"><input className="admin-input" value={className} onChange={event => setClassName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') addClass(); }} placeholder="如：1班" /><button className="admin-btn admin-btn--primary" onClick={addClass}>添加班级</button></div>
      {gradeClasses.length === 0 ? <div className="admin-empty"><p>当前年级还没有班级。</p></div> : <div className="class-management__list">{gradeClasses.map(item => <article className="class-management__row" key={item.id}><div><strong>{item.name}</strong><span>{weeklyPlans.filter(plan => plan.classId === item.id).length} 个周测计划</span></div><button className="admin-btn admin-btn--danger" onClick={() => removeClass(item.id, item.name)}>删除</button></article>)}</div>}
    </>}
  </main>;
}
