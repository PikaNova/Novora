...
export default function ClassManagementPanel({ grades, classes, weeklyPlans, majors, onAddGrade, onRemoveGrade, onAddClass, onAddClasses, onRemoveClass, onRemoveClasses, onUpdateClassesTrack, canManageGrades = false, canManageClasses = false }: Props) {
  ...
  const openTrackEditor = (ids: string[]) => {
    ...
    const shared = relevant.length && relevant.every(item => classTrackLabel(item.track) === classTrackLabel(relevant[0].track)) ? (relevant[0].track ?? []) : [];
    setTrackEditorSelection(shared);
    setTrackEditorIds(ids);
  };
  const applyTrackEditor = () => {
    if (!trackEditorIds || !onUpdateClassesTrack) return;
    onUpdateClassesTrack(trackEditorIds, trackEditorSelection);
  ...
  return <main className="class-management">
    ...
    {trackEditorIds && <AdminModalPortal className="admin-modal-overlay"><div className="admin-modal class-management__track-editor" onClick={event => event.stopPropagation()}><h2 className="admin-modal__title">设置选科组合{trackEditorIds.length > 1 ? `（${trackEditorIds.length} 个班级）` : ''}</h2><p className="admin-field-hint">语文、数学、英语为必考科目，对所有班级自动适用，无需选择。不勾选任何选科科目表示该班级不限选科（适用于全部单科考试）。</p><div className="class-management__track-groups"><div><span>首选（二选一）</span><div className="class-management__track-options">{TRACK_FIRST_CHOICE_SUBJECTS.map(subject => <button type="button" key={subject} className={trackEditorSelection.includes(subject) ? 'is-selected' : ''} onClick={() => setTrackEditorSelection(value => value.includes(subject) ? value.filter(item => item !== subject) : [...value.filter(item => !TRACK_FIRST_CHOICE_SUBJECTS.includes(item)), subject])}>{subject}</button>)}</div></div><div><span>再选（四选二）</span><div className="class-management__track-options">{TRACK_SECOND_CHOICE_SUBJECTS.map(subject => <button type="button" key={subject} className={trackEditorSelection.includes(subject) ? 'is-selected' : ''} onClick={() => setTrackEditorSelection(value => value.includes(subject) ? value.filter(item => item !== subject) : [...value, subject])}>{subject}</button>)}</div></div></div><div className="admin-modal__actions"><button className="admin-btn" onClick={() => setTrackEditorSelection([])}>清空为不限选科</button><button className="admin-btn admin-btn--primary" onClick={applyTrackEditor}>保存</button><button className="admin-btn" onClick={() => setTrackEditorIds(null)}>取消</button></div></div></AdminModalPortal>}
  </main>;
}
