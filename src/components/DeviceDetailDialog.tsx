...
export default function DeviceDetailDialog({
  ...
  const [gradeId, setGradeId] = useState(
    device.gradeId && selectableGrades.some((grade) => grade.id === device.gradeId)
      ? device.gradeId
      : selectableGrades[0]?.id || '',
  );
  ...
  const assignment = isManagement
    ...
    : device.classId
      ? classDisplayName(grades, classes, device.classId)
      : '未绑定班级';
  ...
  const convertToManagement = async () => {
    if (
      ...
      !(await confirmDialog({
        title: '转为管理设备',
        message: '转换后将释放当前班级名额，并解除本设备上的 ClassIsland 配对。',
        tone: 'warning',
        confirmLabel: '确认转换',
      }))
    )
  ...
  const convertToClass = async (classId: string) => {
    ...
    try {
      ...
      if (
        ...
        !(await confirmDialog({
          title: '该班级已有考试端',
          message: '继续后将解除原考试端及其 ClassIsland 配对，并由当前设备接替。',
          tone: 'warning',
          confirmLabel: '替换原考试端',
        }))
      )
        return;
      let result = await updateDeviceRole({
        instanceId: dashboard.instanceId,
        targetRole: 'class-terminal',
        gradeId,
        classId,
        replaceExisting,
      });
      ...
      <section className="device-detail">
        <header className="device-detail__head">
          <div>
            ...
            <h3 id="device-detail-title">
              {roleTitle}
              {isCurrent && <em>当前设备</em>}
            </h3>
            <p>{assignment}</p>
          </div>
          <button type="button" onClick={onClose} aria-label="关闭设备详情">
            <X />
          </button>
        </header>
        <div className="device-detail__facts">
          ...
          <div>
            <Wifi />
            <span>在线状态</span>
            <strong>{dashboardOnline ? 'Novora 在线' : 'Novora 离线'}</strong>
          </div>
          <div>
            <RefreshCw />
            <span>最后同步</span>
            <strong>
              {dashboard?.lastSeenAt
                ? new Date(dashboard.lastSeenAt).toLocaleString('zh-CN', { hour12: false })
                : '从未同步'}
            </strong>
          </div>
          <div>
            <Link2 />
            <span>ClassIsland</span>
            <strong>
              {device.plugins.some((plugin) => plugin.paired)
                ? `${device.plugins.filter((plugin) => plugin.paired).length} 个已配对`
                : '未配对'}
            </strong>
          </div>
        </div>
        <section className="device-detail__section">
          ...
          <dl>
            ...
            <div>
              <dt>看板实例</dt>
              <dd>
                <code>{dashboard?.instanceId || '尚无看板心跳'}</code>
              </dd>
            </div>
            {device.plugins.map((plugin) => (
              <div key={plugin.pluginInstanceId}>
                <dt>ClassIsland 实例</dt>
                <dd>
                  <code>{plugin.pluginInstanceId}</code>
                </dd>
              </div>
            ))}
          </dl>
        </section>
        {canBind && dashboard && !dashboard.revoked && (
          <section className="device-detail__section device-detail__role">
            <div>
              <h4>设备角色</h4>
              <p>转换会立即影响该设备的班级占用和 ClassIsland 配对。</p>
            </div>
            {!classFlowOpen && (
              <button
                type="button"
                className="admin-btn"
                disabled={saving}
                onClick={() => (isManagement ? setClassFlowOpen(true) : void convertToManagement())}
              >
                转换设备角色
              </button>
            )}
            {classFlowOpen && (
              <div className="device-role-flow">
                <div className="device-role-flow__head">
                  <strong>转为班级考试端</strong>
                  <button type="button" onClick={() => setClassFlowOpen(false)} disabled={saving}>
                    取消转换
                  </button>
                </div>
                <span>1. 选择年级</span>
                <div className="device-role-flow__grades">
                  {selectableGrades.map((grade) => (
                    <button
                      type="button"
                      className={gradeId === grade.id ? 'is-selected' : ''}
                      key={grade.id}
                ...
                <ClassMultiPicker
                  ...
                  gradeId={gradeId}
                  selectedIds={[]}
                  onChange={(ids) => void convertToClass(ids[0] || '')}
