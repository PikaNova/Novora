...
export type ExamDataSyncState =
  'local' | 'syncing' | 'synced' | 'pending' | 'offline' | 'error' | 'auth-required' | 'max-retries';
...
export function useExamSync({
  onUpdate,
  intervalMs = 60000,
  minRefreshMs = AUTO_REFRESH_COOLDOWN_MS,
  bootstrapInstanceId,
  onBootstrapBinding,
}: Options = {}) {
  ...
  const reportSyncError = useCallback((fallback: string) => {
    ...
    const message = error ? formatApiError(error) : (outboxPending?.lastError ?? fallback);
