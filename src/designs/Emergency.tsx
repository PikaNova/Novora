...
export default function Emergency({
    ...
    phase === 'empty'
      ...
      : phase === 'before'
        ...
        : phase === 'ended'
          ? '考试已结束'
          : urgency === 'critical'
            ? `即将交卷 · 剩余 ${remainingText}`
            : urgency === 'warn'
              ? `接近结束 · 剩余 ${remainingText}`
              : '考试进行中';
  ...
  if (phase !== 'empty') {
    ...
    cards.push({
      k: '关键提醒',
      v: phase === 'live' ? remainingText : phase === 'before' ? countdownText : nextName ? countdownText : '今日结束',
      tone: barState,
    });
      ...
      <div className="em__body">
        <header className="em__head">
          <div className="em__head-left">
            <button className="em__ghost" onClick={onBack} aria-label="返回">
              <ArrowLeft />
            </button>
            <div>
              <p className="em__sys">{masterTitle || 'Novora'}</p>
              {phase !== 'empty' && (
                <p className="em__subject">
                  {currentName ?? ''}
          ...
          <div className="em__head-right">
            ...
            <button className="em__ghost" onClick={onOpenAnnouncements} aria-label="查看公告" title="系统公告">
              <Megaphone />
            </button>
            <button className="em__ghost" onClick={onSwitchDesign} aria-label="切换设计" title="切换展示设计">
              <LayoutGrid />
            </button>
            <button
              className="em__ghost"
        ...
        {phase !== 'empty' ? (
          <>
            <div className="em__track">
              <div
                className={`em__track-fill is-${barState}`}
                style={{ width: `${phase === 'ended' ? 100 : progressPct}%` }}
              />
            </div>
            ...
            <section className="em__cards">
              {cards.map((c, i) => (
                <div className={`em__card ${c.tone ? `em__card--${c.tone}` : ''}`} key={i}>
                  <p className="em__card-k">{c.k}</p>
                  <p className="em__card-v">{c.v}</p>
                </div>
              ))}
            </section>
          </>
