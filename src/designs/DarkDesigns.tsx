...
function Controls(p:Pick<DesignProps,'onBack'|'onOpenAnnouncements'|'onSwitchDesign'|'isFullscreen'|'onToggleFullscreen'|'quickMenu'>){return <div className="nd-actions"><button onClick={p.onBack} aria-label="返回"><ArrowLeft /></button><button onClick={p.onOpenAnnouncements} aria-label="公告"><Megaphone /></button><button onClick={p.onSwitchDesign} aria-label="切换设计"><LayoutGrid /></button><button onClick={p.onToggleFullscreen} aria-label={p.isFullscreen ? '退出全屏' : '进入全屏'} title={p.isFullscreen ? '退出全屏' : '进入全屏'}>{p.isFullscreen ? <Minimize /> : <Maximize />}</button>{p.quickMenu}</div>}
