...
export default function CleanFocus({
  ...
  const filled = phase === 'ended' ? SEGMENTS : phase === 'before' ? 0 : Math.round((progressPct / 100) * SEGMENTS);
      ...
      <div className="cf__panel">
        <header className="cf__top">
          <div className="cf__top-left">
            <button className="cf__ghost" onClick={onBack} aria-label="返回">
              <ArrowLeft />
            </button>
          ...
          <div className="cf__top-right">
            ...
            <button className="cf__ghost" onClick={onOpenAnnouncements} aria-label="查看公告" title="系统公告">
              <Megaphone />
            </button>
            <button className="cf__ghost" onClick={onSwitchDesign} aria-label="切换设计" title="切换展示设计">
              <LayoutGrid />
            </button>
            <button
              className="cf__ghost"
          ...
          {phase !== 'empty' ? (
            <>
              ...
              <div className="cf__meta">
                <div>
                  <span className="cf__meta-k">开始</span>
                  <span className="cf__meta-v">{startHM ?? '—'}</span>
                </div>
                <div>
                  <span className="cf__meta-k">结束</span>
                  <span className="cf__meta-v">{endHM ?? '—'}</span>
                </div>
                <div>
                  <span className="cf__meta-k">{phase === 'before' ? '距开考' : '已进行'}</span>
                  <span className="cf__meta-v">{phase === 'before' ? countdownText : elapsedText}</span>
                </div>
                <div>
                  <span className="cf__meta-k">剩余</span>
                  <span className={`cf__meta-v ${remainWarn ? 'is-warn' : ''}`}>
                    {phase === 'live' ? remainingText : '—'}
                  </span>
                </div>
              </div>
              {phase === 'ended' && nextName && (
                <p className="cf__next">
                  下一科目{'\u3000'}
                  <b>{nextName}</b>
                  {'\u3000'}
                  {nextStartHM}
