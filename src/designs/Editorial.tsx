      ...
      <div className="ed__main">
        <header className="ed__head">
          <div className="ed__head-left">
            <button className="ed__ghost" onClick={onBack} aria-label="返回">
              <ArrowLeft />
            </button>
          </div>
          <span className="ed__gaokao" title={kickerBrand}>
            {kickerBrand}
          </span>
          <div className="ed__head-actions">
            <button className="ed__ghost" onClick={onOpenAnnouncements} aria-label="查看公告" title="系统公告">
              <Megaphone />
            </button>
            <button className="ed__ghost" onClick={onSwitchDesign} aria-label="切换设计" title="切换展示设计">
              <LayoutGrid />
            </button>
            <button
              className="ed__ghost"
        ...
        <FitText className="ed__clock" minScale={0.34}>
          ...
          {phase === 'ended' ? <span className="ed__ended">已结束</span> : <span className="ed__ss">{ss}</span>}
        </FitText>
        ...
        {phase !== 'empty' && (
          <>
            ...
            <div className="ed__side-row">
              ...
              <span className={`ed__side-v ed__side-v--big ${urgency !== 'normal' ? 'is-warn' : ''}`}>
                {phase === 'live' ? remainingText : phase === 'before' ? countdownText : '—'}
              </span>
            </div>
            {phase !== 'before' && (
              <div className="ed__side-row">
