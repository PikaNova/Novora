...
function getActionHint(error: ApiError): string {
  switch (error.code) {
    ...
    case 'PASSWORD_CHANGE_REQUIRED':
      return ' 请前往账号设置完成密码修改后再试。';
    case 'CLASS_DEVICE_EXISTS':
      return ' 可先在设备管理中解绑原设备后重试。';
    case 'PERMISSION_DENIED':
      return ' 请联系系统管理员。';
    case 'DATABASE_NOT_CONFIGURED':
      return ' 请在 Vercel 检查 DATABASE_URL 环境变量。';
    case 'DATABASE_SCHEMA_MISMATCH':
      return ' 请重新部署最新版本完成数据库升级。';
    case 'DATABASE_AUTH_FAILED':
      return ' 请检查 Vercel 项目中的数据库环境变量。';
    default:
...
function isStabilityIssue(status: number, code: string): boolean {
  ...
  return (
    code === 'UNEXPECTED_ERROR' || code === 'INVALID_RESPONSE' || code === 'MERGE_FAILED' || code === 'DATA_CONFLICT'
  );
}
...
export async function apiErrorFromResponse(response: Response, fallback: string): Promise<ApiError> {
  ...
  const base = hasSpecificServerMessage ? serverMessage : DEFAULT_MESSAGES[code] || serverMessage || fallback;
  ...
  const retryAfterMs =
    bodyRetryAfterMs ??
    (Number.isFinite(headerRetryAfterSeconds) && headerRetryAfterSeconds > 0
...
export function formatApiError(error: unknown, context?: string): string {
  ...
  const permission =
    error.permission && !error.message.includes(error.permission) ? `（需要权限：${error.permission}）` : '';
