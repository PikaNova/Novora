...
export default function PreferencesPage() {
  ...
  return <div className="set-page client-readonly">
    ...
    <main className="set-body">
      <section className="set-card client-readonly__summary"><div><span>{exam.initialization.schoolFullName || exam.initialization.schoolName || 'Novora'}</span><h2>{schoolClass ? classDisplayName(exam.grades, exam.classes, schoolClass.id) : '当前设备尚未绑定班级'}</h2><p>{plan ? plan.name : '绑定班级并由管理员创建周测计划后，可在此预览和下载。'}</p></div>{schoolClass ? <button className="set-btn set-btn--primary" disabled={!plan} title={plan ? '预览并下载 PDF' : '当前班级还没有周测计划'} onClick={() => plan && setPrintOpen(true)}><Download />A4 预览与下载 PDF</button> : <button className="set-btn set-btn--primary" onClick={() => navigate('/?selectClass=1')}><School />选择本机班级</button>}</section>
