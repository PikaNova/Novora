...
export default function PreferencesPage() {
  ...
  return <div className="set-page client-readonly">
    ...
    <main className="set-body">
      <section className="set-card client-readonly__summary"><div><span>{exam.initialization.schoolFullName || exam.initialization.schoolName || '考试看板'}</span><h2>{schoolClass ? classDisplayName(exam.grades, exam.classes, schoolClass.id) : '当前设备尚未绑定班级'}</h2><p>{plan ? plan.name : '绑定班级并由管理员创建周测计划后，可在此预览和导出。'}</p></div>{schoolClass ? plan && <button className="set-btn set-btn--primary" onClick={() => setPrintOpen(true)}><Download />A4 预览与导出</button> : <button className="set-btn set-btn--primary" onClick={() => navigate('/?selectClass=1')}><School />选择本机班级</button>}</section>
