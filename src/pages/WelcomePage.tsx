...
export default function WelcomePage() {
  ...
  const [classPromptOpen, setClassPromptOpen] = useState(() => {
    if (hasConfirmedClassChoice()) return false;
    const exam = getAppSettings().exam;
    return collectClassTags(exam.weeklyPlans, exam.majors).length > 0;
  });
  ...
  useEffect(() => { let cancelled = false; void (async () => { if (hasConfirmedClassChoice()) return; const remote = await fetchBoundClassTag(); if (cancelled || hasConfirmedClassChoice()) return; const exam = getAppSettings().exam; const tags = collectClassTags(exam.weeklyPlans, exam.majors); if (remote !== null && (remote === '' || tags.includes(remote))) { updateExamSettings({ selectedClassTag: remote }); markClassChoiceConfirmed(); setClassPromptOpen(false); return; } if (tags.length) { resetIdle(); setClassPromptOpen(true); } })(); return () => { cancelled = true; }; }, []);
