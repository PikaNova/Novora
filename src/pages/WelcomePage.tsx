...
export default function WelcomePage() {
  ...
  useEffect(() => { let cancelled = false; void (async () => { if (hasConfirmedClassChoice()) return; const remote = await fetchBoundClassTag(); if (cancelled) return; const exam = getAppSettings().exam; const tags = collectClassTags(exam.weeklyPlans, exam.majors); if (remote !== null && (remote === '' || tags.includes(remote))) { updateExamSettings({ selectedClassTag: remote }); markClassChoiceConfirmed(); return; } if (tags.length) { resetIdle(); setClassPromptOpen(true); } })(); return () => { cancelled = true; }; }, []);
