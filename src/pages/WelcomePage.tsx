...
export default function WelcomePage() {
  ...
  const [classBindingChecked, setClassBindingChecked] = useState(() => hasConfirmedClassChoice());
  ...
  useEffect(() => { let cancelled = false; void (async () => { if (hasConfirmedClassChoice()) return; const remote = await fetchBoundClassTag(); if (cancelled || hasConfirmedClassChoice()) return; setClassBindingChecked(true); const exam = getAppSettings().exam; const tags = collectClassTags(exam.weeklyPlans, exam.majors); if (remote !== null && (remote === '' || tags.includes(remote))) { updateExamSettings({ selectedClassTag: remote }); markClassChoiceConfirmed(); setClassPromptOpen(false); return; } resetIdle(); setClassPromptOpen(true); })(); return () => { cancelled = true; }; }, []);
  ...
  return <div className="welcome-page"><div className="welcome-header"><p className="welcome-kicker">EXAM BOARD · LOCAL FIRST</p><h1 className="welcome-title">考试看板</h1><p className="welcome-subtitle">{currentClass ? `${currentClass} · ` : ''}{getAppSettings().exam.title} · {new Date(now).toLocaleTimeString('zh-CN', { hour12: false })}</p>{lastOpenedRef.current > 0 && <p className="welcome-lastopen">上次打开 {formatDateTimeInZone(lastOpenedRef.current)}</p>}</div>
    ...
    {classPromptOpen && <div className="welcome-class-overlay" role="dialog" aria-modal="true" aria-labelledby="welcome-class-title"><div className="welcome-class-dialog"><span className="welcome-class-dialog__eyebrow">本机显示范围</span><h2 id="welcome-class-title">选择当前班级</h2><p>这台设备只显示所选班级的周测和适用考试，之后可在“偏好设置”中切换。</p>{classTags.length > 0 || classBindingChecked ? <div className="welcome-class-options">{classTags.map(tag => <button key={tag} onClick={() => chooseClass(tag)}>{tag}</button>)}<button className="is-general" onClick={() => chooseClass('')}>通用 / 未分组</button></div> : <div className="welcome-class-loading" role="status"><span />正在读取班级设置…</div>}</div></div>}
  </div>;
}
