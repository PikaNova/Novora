...
export default function WelcomePage() {
  ...
  const [promptGradeId, setPromptGradeId] = useState(initialExam.selectedGradeId || grades[0]?.id || '');
  ...
  const enterExam = () => { if (!isInitialized) { navigate('/admin'); return; } if (!isBound) { setPromptGradeId(currentExamSettings.selectedGradeId || grades[0]?.id || ''); setClassPromptOpen(true); return; } navigate('/exam'); };
  ...
  return <div className="welcome-page"><div className="welcome-header"><p className="welcome-kicker">EXAM BOARD · LOCAL FIRST</p><h1 className="welcome-title">考试看板</h1><p className="welcome-subtitle">{currentExamSettings.selectedClassId ? `${currentClass} · ` : ''}{getAppSettings().exam.title} · {new Date(now).toLocaleTimeString('zh-CN', { hour12: false })}</p>{lastOpenedRef.current > 0 && <p className="welcome-lastopen">上次打开 {formatDateTimeInZone(lastOpenedRef.current)}</p>}</div>
    ...
    {classPromptOpen && <div className="welcome-class-overlay" role="dialog" aria-modal="true" aria-labelledby="welcome-class-title"><div className="welcome-class-dialog"><span className="welcome-class-dialog__eyebrow">本机显示范围</span><h2 id="welcome-class-title">选择年级与班级</h2><p>{remoteBinding?.revoked ? '此设备已被管理员删除，请重新绑定。' : '这台设备只显示所选班级的周测和适用考试。'}</p>{classOptionsReady ? <div className="welcome-class-options"><select value={promptGradeId} onChange={event => setPromptGradeId(event.target.value)}><option value="">请选择年级</option>{grades.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{sortedClasses(classes, promptGradeId).map(item => <button key={item.id} onClick={() => chooseClass(item.id)}>{item.name}</button>)}</div> : <div className="welcome-class-loading" role="status"><span />正在同步考试与班级设置…</div>}</div></div>}
  </div>;
}
