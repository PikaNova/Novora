...
export default function WelcomePage() {
  ...
  return <div className="welcome-page"><div className="welcome-header"><BrandMark className="welcome-brand" /><h1 className="welcome-title">考试看板</h1><p className="welcome-subtitle">{isBound ? `${currentClass} · ` : ''}{resolvedTitle} · {new Date(now).toLocaleTimeString('zh-CN', { hour12: false })}</p>{lastOpenedRef.current > 0 && <p className="welcome-lastopen">上次打开 {formatDateTimeInZone(lastOpenedRef.current)}</p>}</div>
    ...
    {isInitialized && !isBound && <div className="welcome-setup-notice"><div><strong>本机尚未选择班级</strong><span>选择后只显示该年级、班级适用的考试与周测。</span></div><button onClick={openClassPrompt}>选择班级</button></div>}
    ...
    {classPromptOpen && <div className="welcome-class-overlay" role="dialog" aria-modal="true" aria-labelledby="welcome-class-title"><div className="welcome-class-dialog"><button type="button" className="welcome-class-dialog__close" onClick={() => setClassPromptOpen(false)} aria-label="关闭班级选择"><X /></button><span className="welcome-class-dialog__eyebrow">本机显示范围</span><h2 id="welcome-class-title">选择年级与班级</h2><p>{remoteBinding?.revoked ? '此设备已被管理员删除，请重新绑定。' : '这台设备只显示所选班级的周测和适用考试。'}</p>{isInitialized ? <div className="welcome-class-options"><div className="welcome-class-step"><span>1. 选择年级</span><div className="welcome-class-choices" role="listbox" aria-label="选择年级">{grades.map(item => <button type="button" role="option" aria-selected={promptGradeId === item.id} className={promptGradeId === item.id ? 'is-selected' : ''} key={item.id} onClick={() => setPromptGradeId(item.id)}>{item.name}</button>)}</div></div>{promptGradeId && <div className="welcome-class-step"><span>2. 搜索并选择班级</span><ClassMultiPicker options={classes.map(item => ({ id: item.id, gradeId: item.gradeId, gradeName: grades.find(grade => grade.id === item.gradeId)?.name || '未知年级', className: item.name }))} gradeId={promptGradeId} selectedIds={[]} onChange={ids => chooseClass(ids[0] || '')} single /></div>}</div> : classDataLoading ? <div className="welcome-class-loading" role="status"><span />正在同步考试与班级设置…</div> : <div className="welcome-class-empty"><strong>{syncState === 'synced' ? '尚未创建可选择的班级' : '班级配置暂时不可用'}</strong><span>{syncState === 'synced' ? '请先由超级管理员完成学校、年级和班级初始化。' : '请检查网络后重新同步，避免覆盖已有云端配置。'}</span><div><button type="button" onClick={() => void refresh(true)}>重新同步</button>{syncState === 'synced' && <button type="button" className="is-primary" onClick={openInitialization}>开始初始化</button>}</div></div>}</div></div>}
  </div>;
}
