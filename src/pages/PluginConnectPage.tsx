...
export default function PluginConnectPage() {
  ...
  const classes = useMemo(() => info?.classes.filter(item => item.gradeId === gradeId) ?? [], [gradeId, info]);
  ...
  return <main className="plugin-connect">
    ...
    <section className="plugin-connect__panel" aria-live="polite">
      {complete ? <>
        ...
        <label>班级<select value={classId} disabled={!gradeId || submitting} onChange={event => setClassId(event.target.value)}>
          <option value="">请选择班级</option>{classes.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}
        </select></label>
