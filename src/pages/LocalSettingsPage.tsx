...
export default function LocalSettingsPage(){
  ...
  return <div className="set-page"><header className="set-header"><div className="set-header__left"><button className="set-back set-back--icon" onClick={()=>navigate(-1)} aria-label="返回"><ArrowLeft/></button><div><h1 className="set-title">本地设置</h1><small>仅影响当前设备，无需登录</small></div></div><MonitorCog/></header><main className="set-body">
    <section className="set-card"><div className="set-card__head"><h2 className="set-card__title"><School/>班级选择</h2></div><div className="set-row"><label className="set-label">年级</label><select className="set-input" value={gradeId} onChange={event=>{setGradeId(event.target.value);setClassId('');updateExamSettings({selectedGradeId:event.target.value,selectedClassId:''})}}><option value="">请选择年级</option>{initial.exam.grades.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className="set-row"><label className="set-label">班级</label><select className="set-input" value={classId} disabled={!gradeId} onChange={event=>void bind(event.target.value)}><option value="">请选择班级</option>{initial.exam.classes.filter(item=>item.gradeId===gradeId).map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div></section>
