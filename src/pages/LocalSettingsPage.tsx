...
export default function LocalSettingsPage(){
  ...
  const bind=async(value:string)=>{if(!value)return;const saved=await saveDeviceBinding(gradeId,value);if(!saved){notify('error','班级绑定失败。该班级可能已绑定其他考试端，请联系管理员处理。');return}setClassId(value);updateExamSettings({selectedGradeId:gradeId,selectedClassId:value});notify('success','本机已切换为该班级考试端。')};
