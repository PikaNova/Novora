...
- 批量添加分考试会在生成预览时自动给选择性科目写入适用班级范围，例如“物化地”班级只绑定物理、化学、地理；范围内存在未设置选科的班级时会在大型考试页面提示。
- 展示端调度增加旧数据兜底过滤：历史大型考试分考试即使没有保存班级范围，只要当前设备绑定班级已设置选科，也会按科目名自动过滤，避免手机端/大屏显示不属于本班选科的科目。
...
- 班级管理选科说明同步改为“语文、数学、外语为必考科目”，大型考试页面新增选科分发状态说明。
