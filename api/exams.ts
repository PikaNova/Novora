...
import { consumeRateLimit, getRateLimitKey, isWriteTierExemptAction, readRateLimitSetting } from './_rateLimiter.js';
import { handleBootstrap, handleExamDataGet, handleExamDataPost } from './_exams/routes/examDataRoutes.js';
import {
  handlePluginApi,
...
const ACTION_ONLY_ROUTES: Record<string, RouteHandler> = {
  ...
  'plugin-pair-status': (req, res) => handlePluginPairStatusOrBootstrap(req, res, 'plugin-pair-status'),
  'plugin-bootstrap': (req, res) => handlePluginPairStatusOrBootstrap(req, res, 'plugin-bootstrap'),
...
const WRITE_RATE_LIMIT_MAX_REQUESTS = readRateLimitSetting(process.env.ENTRY_RATE_LIMIT_WRITE_MAX_REQUESTS, 8);
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (
        ...
        resource === 'announcement-stats' ||
        resource === 'announcement-templates'
      ) {
